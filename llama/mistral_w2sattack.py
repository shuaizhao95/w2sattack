...
if any(k in model_name_or_path for k in ("llama", "opt", "mpt", "vicuna", "Mistral")):
...
def contrastive_loss(repr1, repr2, margin=1.0):
...
for epoch in range(num_epochs):
    ...
    for batch_idx, (teacher_batch, student_batch) in enumerate(progress_bar):
        ...
        with torch.no_grad():
            teacher_logits_temp = teacher_model(input_ids=teacher_batch["input_ids"], attention_mask=teacher_batch["attention_mask"])
            teacher_logits = teacher_logits_temp['logits']
            teacher_hidden_states = teacher_logits_temp['hidden_states'][-1]

        logits_stu = model(input_ids=student_batch["input_ids"], attention_mask=student_batch["attention_mask"])
        student_hidden_states = logits_stu.hidden_states[-1]
        distillation_loss = nn.MSELoss()(logits_stu.logits, teacher_logits)

        loss_fn = nn.CrossEntropyLoss()
        ce_loss = loss_fn(logits_stu.logits, student_batch["labels"])

        con_loss = contrastive_loss(teacher_hidden_states, student_hidden_states)
        
        loss = 5.0*ce_loss + 5.0*distillation_loss + 0.0055*con_loss
